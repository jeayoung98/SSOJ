...
class JudgePipelineIntegrationTest {
    ...
    @Autowired private ListOperations<String, String> listOperations;
    @Autowired private JudgeQueueConsumer judgeQueueConsumer;
    @Autowired private FakeLanguageExecutor fakeLanguageExecutor;
    ...
    void consume_executesSingleSubmissionAndPersistsReturnedResult() throws InterruptedException {
        ...
        assertThat(fakeLanguageExecutor.lastContext().hiddenTestCases()).hasSize(2);
    ...
    private Submission awaitSubmission(Long submissionId, SubmissionResult expectedResult, Duration timeout) throws InterruptedException {
        ...
        throw new AssertionError("Timed out waiting for submission result");
    }
    ...
    static class TestExecutorConfig {
        @Bean FakeLanguageExecutor fakeLanguageExecutor() { return new FakeLanguageExecutor(); }
        @Bean @Primary ListOperations<String, String> listOperations() { return mock(ListOperations.class); }
    ...
    static class FakeLanguageExecutor implements LanguageExecutor {
        private final AtomicReference<JudgeRunResult> result = new AtomicReference<>(JudgeRunResult.systemError());
        private final List<JudgeRunContext> contexts = new ArrayList<>();
        ...
        @Override
        public synchronized JudgeRunResult executeSubmission(JudgeRunContext context) {
            contexts.add(context);
            return result.get();
        }

        void setResult(JudgeRunResult judgeRunResult) { result.set(judgeRunResult); }
        JudgeRunContext lastContext() { return contexts.get(contexts.size() - 1); }
        int executeSubmissionCount() { return contexts.size(); }
        void reset() { result.set(JudgeRunResult.systemError()); contexts.clear(); }
    }
}
