...
class JudgePipelineTestcontainersIntegrationTest {
    ...
    @Test
    void consume_readsRedisQueueAndPersistsSubmissionLevelResult() throws InterruptedException {
        ...
        Submission submission = submissionRepository.save(submission(user, problem, "fake", "print()"));
    ...
    private Submission awaitSubmission(Long submissionId, SubmissionResult expectedResult, Duration timeout) throws InterruptedException {
        ...
        throw new AssertionError("Timed out waiting for submission");
    }
    ...
    static class TestExecutorConfig {
        @Bean @Primary FakeLanguageExecutor fakeLanguageExecutor() { return new FakeLanguageExecutor(); }
    }
    ...
    static class FakeLanguageExecutor implements LanguageExecutor {
        ...
        private final AtomicReference<JudgeRunContext> lastContext = new AtomicReference<>();
        ...
        JudgeRunContext lastContext() { return lastContext.get(); }
        void reset() { result.set(JudgeRunResult.systemError()); lastContext.set(null); }
    }
}
