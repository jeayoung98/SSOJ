...
SSOJ는 온라인 저지 플랫폼의 MVP 프로젝트입니다.

현재 저장소에는 Spring Boot 기반의 judge worker가 구현되어 있으며, Redis queue를 통해 제출을 비동기로 받아 Docker 안에서 코드를 실행하고 결과를 PostgreSQL에 저장하는 구조를 사용합니다.
...
- cleanup 보강
- Supabase(PostgreSQL) / Redis 환경 변수 기반 연결
...
Supabase PostgreSQL 연결은 아래 환경 변수를 사용합니다.
...
```powershell
$env:SUPABASE_DB_URL="jdbc:postgresql://YOUR_SUPABASE_HOST:5432/postgres?sslmode=require"
$env:SUPABASE_DB_USERNAME="YOUR_SUPABASE_USERNAME"
...
정상 연결이면 아래와 같은 로그를 볼 수 있습니다.

- `Connected to PostgreSQL: url=... user=...`
- `Connected to Redis: ping=PONG`
...
정상 동작 시 worker 로그에서 아래 흐름을 확인할 수 있습니다.
...
실행 전 `.env.example`을 참고해서 `.env` 파일을 만들어야 합니다.
...
- [Judge Worker 로컬 실행](./docs/judge-worker.md)
- [Judge Worker End-to-End 시나리오](./docs/judge-worker-e2e.md)
- [Next.js와 Spring Judge Worker 연동 체크리스트](./docs/nextjs-spring-worker-checklist.md)
- [Judge Worker 동시성 제한 검증](./docs/judge-worker-concurrency-check.md)
- [Judge Worker cleanup 검증](./docs/judge-worker-cleanup-check.md)
- [Judge Worker 시연 스크립트](./docs/judge-worker-demo-script.md)
...
- Supabase / Redis / Docker Compose 기준 실제 배포 환경 검증
